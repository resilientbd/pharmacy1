...
//		ps5.setString(2,"dis");
//		ps5.executeUpdate();
 String sql="CREATE TABLE IF NOT EXISTS NEWCUST4(ID INTEGER,USERNAME TEXT,PASSWORD TEXT, REPASSWORD TEXT,DISNAME TEXT, ADDERESS TEXT,CITYNAME TEXT,STATENAME TEXT,COUNTRY TEXT,REGION TEXT,PHONE INTEGER,EMAIL TEXT,PRIMARY KEY (ID))";
        Statement ps=con.createStatement();
		ps.executeUpdate(sql);
		 sql="CREATE TABLE IF NOT EXISTS  NEWORDER4(ID INTEGER,PRODCODE TEXT,PRODUCTNAME TEXT,TAX INTEGER,MINQ INTEGER,ORDERQ INTEGER,DISCOUNT INTEGER,NETCOST INTEGER,AMOUNT INTEGER,PAYMODE TEXT,PRIMARY KEY (ID))";
		
		ps.executeUpdate(sql);
		sql="CREATE TABLE IF NOT EXISTS PHARMACYADMIN(USERNAME TEXT,PASSWORD TEXT)";
	
		ps.executeUpdate(sql);
		PreparedStatement ps4 = con.prepareStatement("Insert into PHARMACYADMIN values(?,?)");
	    ps4.setString(1,"admin");
		ps4.setString(2,"admin");
		
		ps4.executeUpdate();
		
		 sql="CREATE TABLE  IF NOT EXISTS DISTRIBUTERADMIN(USERNAME TEXT,PASSWORD TEXT)";
		ps.executeUpdate(sql);
		
		PreparedStatement ps5 = con.prepareStatement("Insert into DISTRIBUTERADMIN values(?,?)");
		ps5.setString(1,"dis");
		ps5.setString(2,"dis");
		ps5.executeUpdate();
